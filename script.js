document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateHighlight() {
    ...
    if (currentCategoryIndex >= 0 && currentLinkIndex >= 0) {
      const currentLinks =
        categories[currentCategoryIndex].querySelectorAll(".links a");
  ...
  function updateMobileGreeting() {
    if (mobileGreeting) {
      ...
      document
        ...
        .style.setProperty("--greeting", `"${greeting}"`);
    }
  }
...
└──────────────────────────────────┘`;
  ...
  function handleSearch(query) {
    if (query.startsWith("!g ")) {
      ...
      addToCommandHistory(
        `!y ${searchTerm}`,
        `Searching YouTube for "${searchTerm}"`,
      );
      ...
      addToCommandHistory(
        `!w ${searchTerm}`,
        `Searching Wikipedia for "${searchTerm}"`,
      );
        ...
        case "k":
          ...
          if (currentCategoryIndex === -1) {
            ...
            const currentLinks =
              categories[currentCategoryIndex].querySelectorAll(".links a");
            currentLinkIndex =
              (currentLinkIndex - 1 + currentLinks.length) %
              currentLinks.length;
          }
          updateHighlight();
          break;
        case "h":
          ...
          if (currentCategoryIndex === -1) {
            ...
            const previousCategoryIndex =
              ...
              categories.length;
            const previousLinks =
              categories[previousCategoryIndex].querySelectorAll(".links a");
            currentLinkIndex = Math.min(
              currentLinkIndex,
              previousLinks.length - 1,
            );
        ...
        case "l":
          ...
          if (currentCategoryIndex === -1) {
            ...
            const nextCategoryIndex =
              (currentCategoryIndex + 1) % categories.length;
            const nextLinks =
              categories[nextCategoryIndex].querySelectorAll(".links a");
  ...
  document.getElementById("darkModeToggle").addEventListener("click", () => {
    ...
    localStorage.setItem(
      "ultraDarkMode",
      document.body.classList.contains("ultra-dark"),
    );
  });
  ...
  // Force focus to the page body to steal focus from address bar
  setTimeout(() => {
    document.body.focus();
    document.body.tabIndex = -1; // Make body focusable
    document.body.focus();

    // Also try focusing a hidden input as fallback
    const focusHelper = document.createElement("input");
    focusHelper.style.position = "absolute";
